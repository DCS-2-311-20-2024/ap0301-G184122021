...
cardboardRobot.add(legL);
  ...
  body.position.y=legLen+bodyD/2;
...
armR.position.set(bodyW - armW, legLen - bodyH + armLen, 0);
cardboardRobot.add(armR);
const armL = new THREE.Mesh(armGeometry,cardboardMaterial);
armL.position.set(-bodyW +armW,legLen-bodyH+armLen,0);
...
head.position.y=legLen+bodyH+headH/4;
...
eyeL.position.set(-eyeSep/2, eyePos, headD/2+0.04);
head.add(eyeL);
const eyeR = new THREE.Mesh(circleGeometry,blackMaterial);
eyeR.position.set(eyeSep/2, eyePos,headD/2+0.04);
head.add(eyeR);
const triangleGeometry = new THREE.BufferGeometry();
  const triangleVertices = new Float32Array( [
